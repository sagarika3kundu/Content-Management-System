...
if($_REQUEST['flag'] == "1")
{
	...
	mysql_query("insert into `gallery` set `image_desc` ='$_REQUEST[image_desc]', `image_thumb` = '$picname', `image_big` = '$upname'");
	echo "<script>location.href='manage_homepro.php?msg=1'</script>";
}
if($_REQUEST['flag'] == "2")
{
	if($_FILES['gallery_image']['name'] != "")
	{
		...
		mysql_query("update `gallery` set `image_thumb` = '$picname', `image_big` = '$upname' where `image_id` = '$_REQUEST[image_id]'");
	}
	mysql_query("update `gallery` set `image_desc` = '$_REQUEST[image_desc]' where `image_id` = '$_REQUEST[image_id]'");
	echo "<script>location.href='manage_homepro.php?msg=2'</script>";
}
if($_REQUEST['flag'] == "delete")
{
	mysql_query("delete from `gallery` where `image_id` = '$_REQUEST[del_id]'");
	echo "<script>location.href='manage_homepro.php?msg=3'</script>";
}
?>
...
<div id="page">
   ...
   {
   		$show=mysql_fetch_array(mysql_query("select * from `gallery` where image_id = '$_REQUEST[edit_id]'"));
   ?>
        <div class="section">
  ...
  <?php
	...
	{
		$sql=mysql_query("select * from `gallery` order by `image_id`");
  ?>                     
     	<div class="section">
    		<div class="title_wrapper">
        		<h2>Manage CMS</h2>
                            ...
                            <div class="sct_left">
                              ...
                              ?>       
                                            ...
                                            <tr class="first">
                                                ...
                                                <td style="width: 96px;">
                                                    <div class="actions_menu">
                                                        <ul>
                                                            <li><a class="edit" href="manage_homepro.php?flag=edit&edit_id=<?php echo $show['image_id']; ?>">Edit</a></li>
                                                            <li><a class="delete" href="manage_homepro.php?flag=delete&del_id=<?php echo $show['image_id']; ?>" onclick="return confirm('Are you sure to delete this?')">Delete</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>	
                                    ...
                                    <div class="table_menu">
                                       <ul class="left">
                                            <li><a href="manage_homepro.php?flag=add" class="button add_new"><span><span>ADD NEW</span></span></a></li>
                                        </ul>
                                    </div>
                                    </fieldset>
                                    </form>
